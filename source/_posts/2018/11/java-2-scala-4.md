---
title: 从 Java 到 Scala（四）：trait vs Java interface
author: Rhyme
tags:
- Java
- ^Rhyme
...
- trait
- ~从 Java 到 Scala
description: trait在尝试着将抽象更好地融为一个整体。
date: 2018-11-27
---

`trait`特质，一个我们既熟悉又陌生的特性。熟悉是因为你会发现它和你平时在Java中使用的`interface接口`有着很大的相似之处，而陌生又是因为`trait`的新玩法会让你打破对原有接口的认知，进入一个更具有挑战性，玩法更高级的领域。所以，在一开始，我们可以对`trait`有一个初步的认识：它是一个加强版的`interface`。之后，随着你对它了解的深入，你就会发现相比Java接口,`trait`跟类更为相似。再之后，你或许会觉察到，**`trait`在尝试着将抽象更好地融为一个整体。**

# 1. trait quick start

在Java中为了避免多重继承所带来的昂贵代价(方法或字段冲突、菱形继承等问题)，Java的设计者们使用了`interface接口`。而为了解决Java接口无法进行`stackable modifications`(无法使用对象状态)、无法提供字段等局限，在Scala中，我们使用`trait`特质而非接口。
...
和Java一样，Scala只支持单继承，但却可以有任意数量的特质。在Scala中，我们不称接口被`implements`实现了，而是`traits`被`mix in`混入了类中。
...
到这里，你或许能够发现，相比`Java interface`,`trait`和类更加相似。而事实也确实如此，`trait`可以具备类的所有特性，除了缺少构造器参数。也就是说你不能想给类传入构造器参数那样给特质传入参数。具体代码这里就不再演示。
...
`trait`最常见的两种使用方式:一种是和Java接口类似，用于设计富接口，另一种是`trait`独有的`stackable modifications`。这里就说到了`interface`和`trait`的第二个区别，`trait`能够使用对象状态，使得它能够对对象状态进行灵活地迭代。
